    ...
    return new Promise((resolve, reject) => {
      ...
      this.once(`${msgId}:finished`, (data) => {
        switch (data.status) {
          ...
          case 'rejected':
            return reject(new Error('MegTron Message Signature: User denied message signature.'))
          default:
            return reject(new Error(`MegTron Message Signature: Unknown problem: ${JSON.stringify(msgParams)}`))
        }
      })
    })
  }
